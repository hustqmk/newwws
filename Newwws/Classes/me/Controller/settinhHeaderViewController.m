...
-(void)updatePhoto:(UIImage *)image
{
    ...
    NSData * imageData = UIImageJPEGRepresentation(image, IMAGE_COMPRESSION_RATIO);
    [BmobProFile uploadFileWithFilename:@"IMAGE.jpg" fileData:imageData block:^(BOOL isSuccessful, NSError *error, NSString *filename, NSString *url, BmobFile *file) {
        if (isSuccessful) {
            BmobUser * bUser = [BmobUser getCurrentUser];
