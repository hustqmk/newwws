...
-(void)landClick
{
    ...
    [BmobUser loginWithUsernameInBackground:user.text password:pwd.text block:^(BmobUser *user, NSError *error) {
        if (error) {
            NSLog(@"%@",error);
            [SVProgressHUD showInfoWithStatus:@"登录失败，请检查用户名和密码是否正确!"];
        }
        else
        {
